...
public class ColorUtil {
    private static final List<ColorName> colorList = new ArrayList<>();

    public static Color getColor(String text) {
        if (text.contains(",")) {
            String[] list = text.split(",");
            return new Color(Integer.parseInt(list[0]), Integer.parseInt(list[1]), Integer.parseInt(list[2]));
        } else {
            for (ColorName name : colorList) {
                if (name.name.equalsIgnoreCase(text)) {
                    return new Color(name.r, name.g, name.b);
                }
            }
        }
        Main.logMessage("[WARN] Invalid color string '" + text + "' - returning Color.BLACK!");
        return Color.BLACK;
    }

    public static Color getColor(long bits) {
        int r = (short) ((int) (bits >> 16)) & 255;
        int g = (short) ((int) (bits >> 8)) & 255;
        int b = (short) ((int) bits) & 255;
    ...
    public static Color getColorDelta(Color end, Color start) {
        return new Color(end.getRed() - start.getRed(), end.getGreen() - start.getGreen(), end.getBlue() - start.getBlue());
    }

    public static Color colorAdd(Color color, int amount) {
        return new Color(color.getRed() + amount, color.getGreen() + amount, color.getBlue() + amount);
    }

    public static Color colorAdd(Color color, Color amount) {
        return new Color(color.getRed() + amount.getRed(), color.getGreen() + amount.getGreen(), color.getBlue() + amount.getBlue());
    }

    public static Color colorMult(Color color, Color amount) {
        return new Color(color.getRed() * amount.getRed(), color.getGreen() * amount.getGreen(), color.getBlue() * amount.getBlue());
    }

    public static Color colorMult(Color color, double amount) {
    ...
    public static Color verify(Color color) {
        ...
        return new Color(
                Math.max(color.getRed(), 0),
                Math.max(color.getGreen(), 0),
                Math.max(color.getBlue(), 0)
        );
    }

    public static List<ColorName> getColorList() {
        if (ColorUtil.colorList.isEmpty()) {
            initColorList();
        }
        return colorList;
    }
